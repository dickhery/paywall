...
const DEFAULT_LOGIN_PROMPT =
  'Payment required. Log in to check access.';
const DEFAULT_PAYMENT_PROMPT = 'Complete payment to continue.';
...
const resolveOptionalText = (value, fallback) => {
...
const idlFactory = ({ IDL }) => {
  const PaywallConfig = IDL.Record({
    ...
    loginPromptText: IDL.Opt(IDL.Text),
    paymentPromptText: IDL.Opt(IDL.Text),
  });
...
const buildOverlay = (price, loginPrompt, onLogin) => {
  ...
  panel.innerHTML = `
    ...
    <p id="paywall-price" style="margin:0 0 12px;font-size:20px;font-weight:600;">Pay ${price} ICP to continue.</p>
    <p id="paywall-prompt" style="margin:0 0 16px;font-size:16px;"></p>
  ...
  const promptLine = panel.querySelector('#paywall-prompt');
  if (promptLine) {
    promptLine.textContent = loginPrompt;
  }
...
const run = async () => {
  try {
    ...
    const loginPrompt = resolveOptionalText(
      config.loginPromptText,
      DEFAULT_LOGIN_PROMPT,
    );
    const paymentPrompt = resolveOptionalText(
      config.paymentPromptText,
    ...
    const overlay = buildOverlay(
      ...
      async () => {
        ...
        try {
          ...
          const promptLine = overlay.querySelector('#paywall-prompt');
          if (promptLine) {
            promptLine.textContent = paymentPrompt;
          }
          ...
          if (userBalanceE8s >= requiredBalanceE8s) {
            ...
            payFromBalanceButton.addEventListener('click', async () => {
              ...
              const confirmMessage = `Are you sure you want to pay ${priceIcp.toFixed(8)} ICP plus network fees? This will unlock the paywall for ${duration}.`;
            ...
            note.textContent =
              `Deposit at least ${requiredBalanceIcp.toFixed(8)} ICP to cover the payment and ledger fees. Copy this Account Identifier into your wallet (e.g., NNS dapp) to send ICP. After transfer, refresh or re-login to see updated balance.`;
